...
function getGoalieSaves(stats) {
  return getStatNumber(stats, ["saves", "sv"]);
}

function getGoalieGoalsAgainst(stats) {
...
function getGoalieShotsFaced(stats) {
  const explicitShotsFaced = getStatNumber(stats, [
    "shots_faced",
    "shotsFaced",
    "sf"
  ]);

  if (explicitShotsFaced) return explicitShotsFaced;

  const saves = getGoalieSaves(stats);
  const goalsAgainst = getGoalieGoalsAgainst(stats);

  return saves + goalsAgainst;
}

function getGoalieGamesPlayed(stats) {
  return getStatNumber(stats, [
    "games_played",
    "gamesPlayed",
    "gp"
...
function getGoalieSavePercent(stats) {
  const explicit = getStatNumber(stats, [
    "save_percent",
    "savePercentage",
    "save_percentage",
    "sv_percent",
  ...
  const saves = getGoalieSaves(stats);
  const shotsFaced = getGoalieShotsFaced(stats);

  if (!shotsFaced) return 0;

  return saves / shotsFaced;
}

function getGoalieGaa(stats) {
  const explicit = getStatNumber(stats, [
    "gaa",
  ...
  const goalsAgainst = getGoalieGoalsAgainst(stats);
  const gamesPlayed = getGoalieGamesPlayed(stats);
...
function formatGoalieSavePercent(value) {
  if (!value) return "—";
...
function getGoalieMetricValue(row, metric) {
  if (metric === "savePercent") return getGoalieSavePercent(row);
  if (metric === "gaa") return getGoalieGaa(row);
  if (metric === "shotsFaced") return getGoalieShotsFaced(row);
  if (metric === "saves") return getGoalieSaves(row);
  if (metric === "gamesPlayed") return getGoalieGamesPlayed(row);
  ...
  const rankedRows = rows
    .map(row => {
      return {
        ...
        value: getGoalieMetricValue(row, metric)
      };
    })
...
function makeDivisionGoalieRankings(leaderData, division) {
  const rows = getDivisionLeaderRows(leaderData, division);

  return {
    savePercent: makeRankMap(rows, "savePercent", "desc"),
    gaa: makeRankMap(rows, "gaa", "asc"),
    shotsFaced: makeRankMap(rows, "shotsFaced", "desc"),
    saves: makeRankMap(rows, "saves", "desc"),
    gamesPlayed: makeRankMap(rows, "gamesPlayed", "desc")
  };
}

function getGoalieRankKey(goalie) {
  if (goalie.slap_id) {
    return `slap:${goalie.slap_id}`;
  }

  return `player:${normalizeKey(goalie.player_display_name)}`;
}

function getGoalieRankingsForPlayer(goalie, divisionRankings) {
  const key = getGoalieRankKey(goalie);

  return {
    savePercent: divisionRankings.savePercent.get(key) || null,
    gaa: divisionRankings.gaa.get(key) || null,
    shotsFaced: divisionRankings.shotsFaced.get(key) || null,
    saves: divisionRankings.saves.get(key) || null,
...
window.loadNextGoalieCard = function loadNextGoalieCard(img) {
  ...
  const card = img.closest(".goalie-player-card");
...
function makeStartingGoalie(rosterData, leaderData, teamId, divisionRankings) {
  ...
  const goalieRows = rosterPlayers.map(player => {
    const stats = getStatsForPlayer(leaderData, teamId, player);
    const saves = getGoalieSaves(stats);
    const shotsFaced = getGoalieShotsFaced(stats);
    const gamesPlayed = getGoalieGamesPlayed(stats);
    const savePercent = getGoalieSavePercent(stats);
    const gaa = getGoalieGaa(stats);
  ...
  const goalie = goalieRows.sort((a, b) => {
    return b.saves - a.saves
        || b.gamesPlayed - a.gamesPlayed
        || b.savePercent - a.savePercent
        || a.player_display_name.localeCompare(b.player_display_name);
    })[0] || null;

    if (goalie && divisionRankings) {
    goalie.ranks = getGoalieRankingsForPlayer(goalie, divisionRankings);
    }
...
function makeTeamView(match, side, data, fallbackColor) {
  ...
  const divisionRankings = makeDivisionGoalieRankings(data.leaders, division);
  ...
  return {
    ...
    goalie: makeStartingGoalie(data.rosters, data.leaders, teamId, divisionRankings)
  };
}
...
function renderBroadcastTopbar(match) {
  ...
  return `
    <header class="broadcast-topbar">
      ...
      <div class="broadcast-title">Starting Goaltenders</div>
...
function renderGoalieStat(label, value, rankInfo, featured = false) {
  return `
    <div class="goalie-stat ${featured ? "featured" : ""}">
      <span class="goalie-stat-label">${escapeHtml(label)}</span>
      <strong class="goalie-stat-value">${escapeHtml(value)}</strong>
      <span class="goalie-stat-rank">(${escapeHtml(formatRank(rankInfo))})</span>
    </div>
  `;
}

function renderGoalieImage(goalie) {
  const candidates = goalie.cardCandidates || [];
  const firstCard = candidates[0] || "";

  return `
    <div class="goalie-player-card ${firstCard ? "" : "missing-card"}">
      ${
        firstCard
          ? `
            <img
              src="${escapeAttr(firstCard)}"
              alt="${escapeAttr(goalie.player_display_name)}"
              data-card-index="0"
              data-card-candidates="${escapeAttr(candidates.join("|"))}"
              onerror="window.loadNextGoalieCard(this);"
            >
          `
          : ""
      }

      <div class="goalie-card-fallback">
        ${escapeHtml(goalie.player_display_name)}
      </div>
    </div>
  `;
}

function renderGoalieCard(team, side) {
  const goalie = team.goalie;
  const sideClass = side === "home" ? "home" : "away";

  if (!goalie) {
    return `
      <section class="goalie-board ${sideClass}">
        ${
          team.logo
            ? `<div class="goalie-watermark" style="background-image: url('${escapeAttr(team.logo)}');"></div>`
            : ""
        }

        <div class="goalie-empty">No goalie data</div>
      </section>
    `;
  }
  ...
  return `
    <section class="goalie-board ${sideClass}">
      ...
      <div class="goalie-card-wrap">
        <div class="goalie-name-bar">
            <strong>${escapeHtml(goalie.player_display_name)}</strong>
        </div>

        <div class="goalie-content">
          ${renderGoalieImage(goalie)}

          <div class="goalie-stat-grid">
            ${renderGoalieStat("SV%", formatGoalieSavePercent(goalie.savePercent), goalie.ranks?.savePercent, true)}
            ${renderGoalieStat("GAA", formatGoalieGaa(goalie.gaa), goalie.ranks?.gaa, true)}
            ${renderGoalieStat("Shots Faced", formatInteger(goalie.shotsFaced), goalie.ranks?.shotsFaced)}
            ${renderGoalieStat("Saves", formatInteger(goalie.saves), goalie.ranks?.saves)}
            ${renderGoalieStat("Games Played", formatInteger(goalie.gamesPlayed), goalie.ranks?.gamesPlayed)}
          </div>
        </div>
      </div>
    </section>
...
function renderGoalieSection(away, home) {
  return `
    <section class="goalie-section">
      ${renderGoalieCard(away, "away")}
      ${renderGoalieCard(home, "home")}
    </section>
  `;
}
...
function renderBroadcastFooter(away, home) {
  return `
    <footer class="broadcast-footer">
      ...
      <div class="footer-center">STARTING GOALTENDERS</div>
      <div class="footer-home">${escapeHtml(home.name)}</div>
    </footer>
  `;
}

function renderGoalieSource(match, data) {
  const container = document.querySelector("#startingGoaliesSource");
  ...
  container.innerHTML = `
    ...
    <div class="broadcast-frame">
      ...
      ${renderGoalieSection(away, home)}
      ${renderBroadcastFooter(away, home)}
    </div>
  `;
}

async function loadStartingGoalies() {
  const matchId = getMatchIdFromUrl();

  if (!matchId) {
    renderGoalieSource(null, {});
    return;
  }
